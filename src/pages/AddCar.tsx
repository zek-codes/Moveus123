    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-8">
        <h2 className="text-base font-semibold text-gray-800 mb-5 flex items-center gap-2">
          <CarFront size={18} className="text-blue-600" /> Add New Vehicle
        </h2>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1.5">Car Name / Model</label>
            <input
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1.5">Number Plate</label>
            <input
      ...
      {cars.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm text-center py-16">
          ...
          <p className="text-sm text-gray-400">No vehicles in the fleet yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {cars.map((car) => (
            <div
              key={car.id}
              className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-3 group relative hover:shadow-md transition-shadow"
            >
              <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center">
                <CarFront size={24} className="text-blue-600" />
              </div>
              <div>
